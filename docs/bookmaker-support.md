...
PRODUCT-015/PRODUCT-016 established that the real upstream source supplies typed `bet-up.it` relay links rather than direct bookmaker URLs. ARCH-007/ADR-0005 then approved one bounded canonical same-origin revisit, implemented by BOOK-019 and certified by SEC-005/QA-003. DEVOPS-013 re-ran exactly once per target bookmaker and both runs stopped safely with coarse `RELAY_INVALID` before bookmaker arrival. The bookmaker-specific deterministic chain therefore remains unobserved. BOOK-021/#156 is merged and its redacted subtype path is Security/QA certified. DEVOPS-014/#161 is the only authorized next live step: exactly one diagnostic re-run per target bookmaker, with no resolver widening, retry, or tuning.
...
PRODUCT-005's generic interactive queue is exhausted. The structured-ingestion path now supports the bounded ADR-0005 relay state machine, but the latest QA-authorized DEVOPS-013 SISAL/BET365 runs still stopped before bookmaker arrival with coarse `RELAY_INVALID`. BOOK-021/#156 now exposes the redacted invalid subtype without changing relay acceptance; DEVOPS-014/#161 is authorized to capture it once per target bookmaker. The matching/security contract remains unchanged, and QA-002/#45 plus production release #46 remain blocked.
