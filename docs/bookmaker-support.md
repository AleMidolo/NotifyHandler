...
| --- | --- | --- | --- |
| SISAL | 1 | Blocked (interactive live); Testable (fixtures) | BOOK-014 consumed a qualifying non-CI BOOK-012 run on `www.sisal.it`. The explorer started on the approved football surface but exhausted its fixed 10-action budget without exposing selector-level event binding, a full-match total-corners market, exact line, requested side, or bound displayed odds. `authorizesProductionMapping` remained false. The deterministic fixture-backed adapter remains Testable. See `docs/live-validation/sisal-book-014.md`. |
...
The passive PRODUCT-004 feasibility queue is exhausted. PRODUCT-005 retains the full-match total-corners target and sequences controlled interactive revalidation through ADMIRALBET, SISAL, and BET365. BOOK-013/ADMIRALBET and BOOK-014/SISAL are both Blocked after qualifying BOOK-012 runs. DEVOPS-010/#96 and DEVOPS-011/#97 are now complete for BET365; BOOK-015/#64 is active and must interpret the actual qualifying BET365 evidence. Matching and safety requirements remain unchanged.
...
The prior BOOK-008 evidence left the **live pre-match football total-corners scope Blocked** pending stronger interactive evidence. A qualifying BOOK-012 BET365 run now exists through DEVOPS-011/#97. It reached the football hub plus World Cup, Champions League, Serie A, Serie B, and Bundesliga competition pages and exposed sanitized event/time groupings, but its `BUDGET_EXHAUSTED` result has not yet been classified by BOOK-015/#64. Until that interpretation is complete, the existing live Blocked/Testable(fixtures) status remains unchanged. `packages/automation` includes the non-CI read-only `live:probe:bet365` command for passive structural summaries. See `docs/live-validation/bet365-book-008.md` and #64/#97 for the interactive handoff.
...
Per PRODUCT-005 sequencing, SISAL/BOOK-014 is Blocked after its qualifying interactive run and PR #95 is merged. BET365/BOOK-015 is now active with a qualifying DEVOPS-011/#97 result awaiting interpretation. If BET365 is also Blocked, Product Coordination must explicitly replan the milestone; the matching/security contract remains unchanged.
