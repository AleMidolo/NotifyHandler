...
| --- | --- | --- | --- |
| SISAL | 1 | Blocked (relay-aware live path); Testable (fixtures) | DEVOPS-014 completed the qualified relay diagnostic and returned `RELAY_INVALID / UNREVIEWED_SAME_ORIGIN_PATH` before SISAL origin arrival, with actions 0 and no live bookmaker evidence. This proves the real relay performs a same-origin HTTPS navigation to a path outside the reviewed `/lnk/<uuid>/<bookmaker>` grammar, but the retained diagnostic intentionally does not expose the actual path template. PRODUCT-026/#167 must obtain a non-sensitive upstream path contract or direct alternative before any new resolver grammar can be reviewed. Fixture-backed behavior remains Testable. See `docs/live-validation/relay-book-016.md`. |
| BET365 | 2 | Blocked (relay-aware live path); Testable (fixtures) | DEVOPS-014 completed the qualified relay diagnostic and returned `RELAY_INVALID / UNREVIEWED_SAME_ORIGIN_PATH` before BET365 origin arrival, with actions 0 and no live bookmaker evidence. This proves the real relay performs a same-origin HTTPS navigation to a path outside the reviewed `/lnk/<uuid>/<bookmaker>` grammar, but the retained diagnostic intentionally does not expose the actual path template. PRODUCT-026/#167 must obtain a non-sensitive upstream path contract or direct alternative before any new resolver grammar can be reviewed. Fixture-backed behavior remains Testable. See `docs/live-validation/relay-book-016.md`. |
...
PRODUCT-015/PRODUCT-016 established that the real upstream source supplies typed `bet-up.it` relay links rather than direct bookmaker URLs. ARCH-007/ADR-0005 approved one bounded canonical same-origin revisit, and BOOK-021 later added a redacted invalid-category diagnostic. DEVOPS-014 has now completed: both SISAL and BET365 returned `UNREVIEWED_SAME_ORIGIN_PATH` before bookmaker arrival. The bookmaker-specific deterministic chain remains unobserved. The retained evidence proves only that the next relay navigation is same-origin HTTPS with no userinfo/query/fragment and a pathname outside the reviewed `/lnk/<uuid>/<bookmaker>` grammar; it does not reveal a reviewable new path template. PRODUCT-026/#167 is therefore the immediate blocker.
...
Therefore the existing deterministic fixture-backed adapter remains **Testable**, while live support remains **Blocked**. DEVOPS-014 now identifies the shared relay blocker precisely as `UNREVIEWED_SAME_ORIGIN_PATH` before SISAL origin arrival. No SISAL event/market/line/side/odds evidence was collected. The actual intermediate pathname grammar remains unknown by design, so widening the resolver would be guesswork. PRODUCT-026/#167 must obtain a sanitized upstream path contract or direct alternative before Architecture can review another finite relay transition. No live selector mapping or implementation issue is created. See `docs/live-validation/sisal-book-014.md` and `docs/live-validation/relay-book-016.md`.
...
Therefore the existing deterministic fixture-backed adapter remains **Testable**, while live support remains **Blocked**. DEVOPS-014 now identifies the shared relay blocker precisely as `UNREVIEWED_SAME_ORIGIN_PATH` before BET365 origin arrival. No BET365 event/market/line/side/odds evidence was collected. The actual intermediate pathname grammar remains unknown by design, so widening the resolver would be guesswork. PRODUCT-026/#167 must obtain a sanitized upstream path contract or direct alternative before Architecture can review another finite relay transition. No live selector mapping or implementation issue is created. See `docs/live-validation/bet365-book-015.md` and `docs/live-validation/relay-book-016.md`.
...
PRODUCT-005's generic interactive queue is exhausted. The structured-ingestion path supports the bounded ADR-0005 state machine, and DEVOPS-014 has now proven that the real upstream next hop uses an unreviewed same-origin pathname for both SISAL and BET365. Because the actual path template is intentionally not retained, there is no evidence-backed resolver change to implement. PRODUCT-026/#167 must supply the upstream path contract/direct alternative or replan the source flow. The matching/security contract remains unchanged, and QA-002/#45 plus production release #46 remain blocked.
