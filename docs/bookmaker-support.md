...
| BET365 | 2 | Blocked (live target scope); Testable (fixtures) | BOOK-027 v1 provenance identified the first transport trigger as `WEBSOCKET_ATTEMPT / SOCKET`. BOOK-031/DEVOPS-018 later retained one sanitized public hostname, `premws-pt1.it.365lpodds.com`, without connecting the socket. BOOK-030 now proposes that hostname as an exact WSS rule only; Security/QA approval and a separately authorized render re-test are still pending. This is transport-configuration evidence only, not matching, feasibility, selector mapping, activation, or live-support evidence. See `docs/live-validation/book-030-bet365-wss-exact-host-rule.md`. |
...
BOOK-031/#221 and DEVOPS-018/#224 subsequently isolated the first source-page WebSocket hostname under the approved no-connect procedure. The validated one-shot artifact identified `premws-pt1.it.365lpodds.com`, with public DNS validation, `socketConnected:false`, and `authorizesPolicy:false`. BOOK-030/#210 therefore proposes only that canonical exact hostname in the BET365 WSS policy. This does not alter the support classification: the rule itself must pass Security/QA review, then a separately authorized render re-test must still establish whether deterministic target evidence becomes available.
