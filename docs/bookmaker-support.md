...
| --- | --- | --- | --- |
| SISAL | 1 | Blocked (relay-aware live path); Testable (fixtures) | BOOK-016 consumed one qualifying real upstream relay run. The shared resolver stopped with `RELAY_REDIRECT_LIMIT` before SISAL origin arrival, so no live bookmaker event/market/line/side/odds evidence was collected. This is a relay-contract blocker, not proof that SISAL lacks the target market. Fixture-backed behavior remains Testable. See `docs/live-validation/relay-book-016.md`. |
| BET365 | 2 | Blocked (relay-aware live path); Testable (fixtures) | BOOK-016 consumed one qualifying real upstream relay run. The shared resolver stopped with `RELAY_REDIRECT_LIMIT` before BET365 origin arrival, so no live bookmaker event/market/line/side/odds evidence was collected. This is a relay-contract blocker, not proof that BET365 lacks the target market. Fixture-backed behavior remains Testable. See `docs/live-validation/relay-book-016.md`. |
...
PRODUCT-015/PRODUCT-016 established that the real upstream source supplies typed `bet-up.it` relay links rather than direct bookmaker URLs. BOOK-016/#104 executed the resulting relay-aware flow for SISAL and BET365, but both one-shot qualifying runs stopped safely with `RELAY_REDIRECT_LIMIT` before bookmaker arrival. The bookmaker-specific deterministic chain therefore remains unobserved. ARCH-007/#147 now owns the relay-contract decision; do not repeat live evidence runs until that decision and any required resolver change are merged.
...
Therefore the existing deterministic fixture-backed adapter remains **Testable**, while live support remains **Blocked**. BOOK-016 later attempted the real upstream relay-aware path and stopped at `RELAY_REDIRECT_LIMIT` before SISAL origin arrival; no SISAL live bookmaker evidence was collected in that run. The current blocker is the shared relay contract, tracked by ARCH-007/#147. No live selector mapping or implementation issue is created. See `docs/live-validation/sisal-book-014.md` and `docs/live-validation/relay-book-016.md`.
...
Therefore the existing deterministic fixture-backed adapter remains **Testable**, while live support remains **Blocked**. BOOK-016 later attempted the real upstream relay-aware path and stopped at `RELAY_REDIRECT_LIMIT` before BET365 origin arrival; no BET365 live bookmaker evidence was collected in that run. The current blocker is the shared relay contract, tracked by ARCH-007/#147. No live selector mapping or implementation issue is created. See `docs/live-validation/bet365-book-015.md` and `docs/live-validation/relay-book-016.md`.
...
PRODUCT-005's generic interactive queue is exhausted. The later structured-ingestion work established a `BETUP_RELAY` source flow, and BOOK-016/#104 has now exercised that path with real upstream SISAL/BET365 samples. Both stopped before bookmaker arrival because the current shared resolver forbids the observed same-origin relay transition. ARCH-007/#147 is the immediate Milestone-6 blocker. The matching/security contract remains unchanged, and QA-002/#45 plus production release #46 remain blocked.
