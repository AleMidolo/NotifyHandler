...
| BET365 | 2 | Blocked (interactive live); Testable (fixtures) | BOOK-015 consumed a qualifying non-CI BOOK-012 run on `www.bet365.it`. Competition, participant, and scheduled-time evidence was exposed, but the explorer exhausted its fixed 10-action budget without establishing deterministic event-to-market navigation, the full-match total-corners market, exact line, requested side, or bound displayed odds. `authorizesProductionMapping` remained false. The deterministic fixture-backed adapter remains Testable. See `docs/live-validation/bet365-book-015.md`. |
