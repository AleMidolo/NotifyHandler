# DEVOPS-010 — Portable Windows BOOK-012 validation runner

Date: **2026-09-21**  
Scope: portable, non-production workstation handoff for the existing BOOK-012 BET365 explorer.

## What this bundle is

The `book012-bet365-diagnostic-v1` GitHub prerelease contains a Windows x64 ZIP that packages the already-reviewed BOOK-012 interactive explorer together with the repository-pinned Node.js runtime, `playwright-core`, and Playwright Chromium revision.

It exists only to reduce workstation setup friction for DEVOPS-011 / #97. It does **not** change bookmaker matching, interaction classification, action budgets, safe-stop behavior, or support status.
...
- default BOOK-012 path: `/hub/it-it/football`;
...
- `run-bet365-validation.cmd` as the one-command Windows launcher.
...
## Operator procedure for DEVOPS-011 / #97

Use a normal Windows x64 workstation with a headed desktop session and ordinary outbound DNS/HTTPS access to `www.bet365.it`.

1. Open the repository Releases page and select prerelease **`book012-bet365-diagnostic-v1`**.
...
5. Run `run-bet365-validation.cmd` by double-clicking it or from cmd/PowerShell.
...
8. Attach **only** `ExplorerSummary.json` to issue #64 for Bookmaker Automation Engineer interpretation.

If DNS/network/runtime prerequisites fail, retain only the minimal error text needed to diagnose the workstation. Such an environment failure is not bookmaker feasibility evidence and must not classify BET365 as `Blocked`.

The existing clean-checkout path remains an alternative:

```text
npm ci --ignore-scripts
npm run live:explore:local -- bet365
```

## Result interpretation

`ExplorerSummary.json` remains non-authorizing evidence. Its `authorizesProductionMapping` field must be `false`. DEVOPS-010 and DEVOPS-011 do not decide whether BET365 is feasible; BOOK-015 / #64 performs that interpretation only after an actual qualifying-host result exists.
