# DEVOPS-008 — Portable Windows BOOK-012 validation runner

Date: **2026-09-21**  
Scope: portable, non-production workstation handoff for the existing BOOK-012 SISAL explorer.

## What this bundle is

The `book012-sisal-diagnostic-v1` GitHub prerelease contains a Windows x64 ZIP that packages the already-reviewed BOOK-012 interactive explorer together with the repository-pinned Node.js runtime, `playwright-core`, and Playwright Chromium revision.

It exists only to reduce workstation setup friction for DEVOPS-009 / #89. It does **not** change bookmaker matching, interaction classification, action budgets, safe-stop behavior, or support status.
...
- default BOOK-012 path: `/scommesse-matchpoint/sport/calcio`;
...
- `run-sisal-validation.cmd` as the one-command Windows launcher.
...
## Operator procedure for DEVOPS-009 / #89

Use a normal Windows x64 workstation with a headed desktop session and ordinary outbound DNS/HTTPS access to `www.sisal.it`.

1. Open the repository Releases page and select prerelease **`book012-sisal-diagnostic-v1`**.
...
5. Run `run-sisal-validation.cmd` by double-clicking it or from cmd/PowerShell.
...
8. Attach **only** `ExplorerSummary.json` to issue #63 for Bookmaker Automation Engineer interpretation.

If DNS/network/runtime prerequisites fail, retain only the minimal error text needed to diagnose the workstation. Such an environment failure is not bookmaker feasibility evidence and must not classify SISAL as `Blocked`.

The existing clean-checkout path remains an alternative:

```text
npm ci --ignore-scripts
npm run live:explore:local -- sisal
```

## Result interpretation

`ExplorerSummary.json` remains non-authorizing evidence. Its `authorizesProductionMapping` field must be `false`. DEVOPS-008 and DEVOPS-009 do not decide whether SISAL is feasible; BOOK-014 / #63 performs that interpretation only after an actual qualifying-host result exists.
