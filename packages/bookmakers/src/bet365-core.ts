...
const DEFAULT_ENTRY = `${BET365_ORIGIN}/`;
...
export class Bet365Adapter implements BookmakerAdapter {
  readonly bookmaker = "bet365" as const;
  readonly supportedOrigins = [BET365_ORIGIN] as const;
  ...
  async prepare(
    ...
    if (target.bookmaker !== this.bookmaker) {
      return failure(ctx, "UNSUPPORTED_BOOKMAKER", "PLAN", "BET365 adapter received a target for another bookmaker.", "RESTART_PLAN");
    }
    ...
    try {
      ...
      return failure(ctx, "UNSAFE_OR_UNSUPPORTED_URL", "NAVIGATION", "BET365 navigation URL is malformed.", "RESTART_PLAN");
    }
    if (
      ...
      return failure(ctx, "UNSAFE_OR_UNSUPPORTED_URL", "NAVIGATION", "BET365 navigation URL must use the approved HTTPS origin without embedded credentials.", "RESTART_PLAN");
    }
    if (!(await ctx.browser.openAllowed(parsed.href)).ok) {
      return failure(ctx, "UNSAFE_OR_UNSUPPORTED_URL", "NAVIGATION", "Browser worker rejected BET365 navigation.", "REOPEN");
    }
    if (cancelled(signal)) return { kind: "CANCELLED" };
    const ready = await ctx.browser.waitForPageReady({ timeoutMs: 15_000 });
    if (!ready.ready) return failure(ctx, "PAGE_LOAD_TIMEOUT", "PAGE_READY", "BET365 page did not reach a supported ready state.", "RETRY");
    ...
    if (!this.supportedOrigins.includes(location.origin as (typeof this.supportedOrigins)[number])) {
      return failure(ctx, "BLOCKED_REDIRECT", "NAVIGATION", "BET365 navigation redirected to an unapproved origin.", "RESTART_PLAN");
    }

    let snapshot = withEvidence(baseEvidence(ctx.evidenceEpoch), {
      origin: matched("BET365_ORIGIN_MATCHED", BET365_ORIGIN, [location.origin]),
    });
    ...
    if (eventCandidates.length === 0) return failure(ctx, "EVENT_NOT_FOUND", "EVENT", "No BET365 event candidates were available.", "RETRY");
    ...
    if (eligibleEvents.length === 0) {
      if (contextUnavailable && participantMatches > 0) return failure(ctx, "EVENT_CONTEXT_UNAVAILABLE", "EVENT", "BET365 event context required by the target is unavailable.");
      return failure(ctx, "EVENT_MISMATCH", "EVENT", "BET365 event identity did not match the target.");
    }
    if (eligibleEvents.length > 1) return failure(ctx, "EVENT_AMBIGUOUS", "EVENT", "More than one BET365 event matched the target.");
    ...
    if (familyContextMatches.length === 0) return failure(ctx, "MARKET_NOT_FOUND", "MARKET", "Requested BET365 market family/context was not found.");
    if (periodMatches.length === 0) {
      if (periodUnavailable) return failure(ctx, "MARKET_CONTEXT_UNAVAILABLE", "MARKET", "Requested BET365 market period evidence was unavailable.");
      return failure(ctx, "MARKET_MISMATCH", "MARKET", "Requested BET365 market period did not match; period substitution is not allowed.");
    }
    if (exactLineMatches.length === 0) return failure(ctx, "LINE_MISMATCH", "LINE", "Requested BET365 market line was not available; neighboring lines are not accepted.");
    if (exactLineMatches.length > 1) return failure(ctx, "LINE_AMBIGUOUS", "LINE", "Multiple BET365 market candidates matched the exact requested line.");
    ...
    if (matchingOutcomes.length === 0) return failure(ctx, "OUTCOME_NOT_FOUND", "OUTCOME", "Requested BET365 outcome side was not found.");
    if (matchingOutcomes.length > 1) return failure(ctx, "OUTCOME_AMBIGUOUS", "OUTCOME", "Multiple BET365 outcomes matched the requested side.");
    ...
    if (selected !== "true") {
      return {
        ...
        failure: {
          ...
          message: "BET365 outcome activation occurred but selected state could not be verified.",
