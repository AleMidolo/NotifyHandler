...
class FixturePage implements BookmakerPagePort {
  ...
  location = { href: "https://www.sisal.it/scommesse-matchpoint/sport/calcio", origin: "https://www.sisal.it" };
  opened: string[] = [];
...
test("prepares an exact SISAL corners selection through the activation gate", async () => {
  ...
  if (result.kind === "READY_FOR_USER") {
    ...
    assert.equal(result.selection.candidate.id, "outcome-over");
  }
});

test("rejects neighboring lines without activating", async () => {
  const nodes = exactNodes();
  nodes.find((node) => node.id === "market-1")!.attrs["data-market-line"] = "10.5";
...
test("fails safely on duplicate matching events", async () => {
  const nodes = exactNodes();
...
test("fails safely when the target market context is not present", async () => {
  const nodes = exactNodes();
...
test("SISAL rejects first-half corners even when family, context, line, side, and odds match", async () => {
  const nodes = exactNodes();
...
test("SISAL fails safely when market period evidence is unavailable", async () => {
  const nodes = exactNodes();
...
test("rejects unapproved deep-link origins before navigation", async () => {
  const page = new FixturePage(exactNodes());
  const gate = new FixtureGate(page);
  const result = await prepare(page, gate, target({ deepLink: "https://bet-up.it/lnk/opaque/sisal" }));
  assert.equal(result.kind, "FAILED_SAFE");
...
test("cancellation prevents selection activation", async () => {
  const page = new FixturePage(exactNodes());
  const gate = new FixtureGate(page);
...
test("post-activation verification failure is explicit", async () => {
  const page = new FixturePage(exactNodes());
...
test("similar but wrong event never activates", async () => {
  const nodes = exactNodes();
...
test("wrong outcome side never activates", async () => {
  const nodes = exactNodes().filter((node) => node.id !== "outcome-over");
  const page = new FixturePage(nodes);
  const gate = new FixtureGate(page);
  const result = await prepare(page, gate);
  assert.equal(result.kind, "FAILED_SAFE");
...
test("duplicate requested outcomes fail as ambiguous", async () => {
  const nodes = exactNodes();
  nodes.push({
    id: "outcome-over-duplicate",
    kind: "outcome",
    parent: "market-1",
    attrs: { "data-outcome-side": "over", "data-odds": "2.08", "aria-pressed": "false" },
  });
...
test("cross-origin redirect is rejected before matching", async () => {
