...
const EPLAY24_ORIGIN = "https://www.eplay24.it";
const DEFAULT_URL = `${EPLAY24_ORIGIN}/scommesse/prematch/calcio/1/palinsesto`;
const MAX_SAMPLES = 12;

interface ProbeSummary {
  readonly bookmaker: "eplay24";
...
function parseApprovedUrl(value: string): URL {
  ...
  if (
    ...
    parsed.origin !== EPLAY24_ORIGIN ||
    parsed.username !== "" ||
    parsed.password !== ""
  ) {
    throw new Error(
      "EPLAY24 live probe only accepts credential-free HTTPS URLs on https://www.eplay24.it.",
    );
  }
  return parsed;
}
...
function sanitizePath(value: string): string {
  try {
    const parsed = new URL(value, EPLAY24_ORIGIN);
    return parsed.origin === EPLAY24_ORIGIN ? parsed.pathname : "[unapproved-origin]";
  } catch {
    return "[invalid-url]";
  }
}
...
async function collectPublicSummary(target: URL, headless: boolean): Promise<ProbeSummary> {
  ...
  try {
    ...
    for (let index = 0; index < await eventLinks.count(); index += 1) {
      ...
      if (!/calcio|football|scommess|prematch|palinsesto|event|match/i.test(`${label} ${href}`)) continue;
      eventLinkCount += 1;
    ...
    return {
      ...
      approvedOrigin: finalUrl.origin === EPLAY24_ORIGIN,
...
export async function runEplay24PublicProbe(
...
async function main(): Promise<void> {
  const configuredUrl = process.env.NH_EPLAY24_LIVE_PROBE_URL;
  const summary = await runEplay24PublicProbe({
    ...(configuredUrl === undefined ? {} : { url: configuredUrl }),
    headless: process.env.NH_EPLAY24_LIVE_PROBE_HEADLESS === "1",
  });
  process.stdout.write(`${JSON.stringify(summary, null, 2)}\n`);
  process.exitCode = 2;
}

const invokedAsScript = process.argv[1]?.endsWith("eplay24-public-probe.ts") ?? false;
if (invokedAsScript) {
  void main().catch((error: unknown) => {
    const message = error instanceof Error ? error.message : "Unknown EPLAY24 probe failure.";
    process.stderr.write(`EPLAY24 public probe failed safely: ${message}\n`);
    process.exitCode = 1;
  });
}
