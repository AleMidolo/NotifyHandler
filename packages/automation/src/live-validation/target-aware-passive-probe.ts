...
function observedOddsFrom(evidence: readonly PassiveSignalEvidence[]): string[] {
  ...
  for (const signal of evidence) {
    for (const snippet of signal.snippets) {
      for (const match of snippet.matchAll(decimal)) {
        ...
        odds.add(String(Number(whole)) + "." + fraction);
...
export async function collectTargetAwarePageEvidence(
  ...
  const displayedOddsCandidates = observedOddsFrom([
    totalCornersMarket,
    fullMatchContext,
    exactLine,
    requestedSideAtLine,
    expectedOdds,
  ]).filter((value) => value !== target.line);
