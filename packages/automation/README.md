...
BOOK-031 adds `npm run live:book030:local`, a source-locked BET365 WSS hostname observer for the SEC-009-approved BOOK-030 evidence procedure. It accepts no target argument, runs headed outside CI with a fresh ephemeral profile, observes only the first WSS attempt, validates only scheme/credentials/default-port/DNS/public-network properties, closes the intercepted socket without establishing it, and may emit only the strict sanitized hostname artifact. It does not mutate the live WSS rule registry and must not be run against the bookmaker until SEC-010 and QA-009 explicitly authorize the single DEVOPS-018 observation.
