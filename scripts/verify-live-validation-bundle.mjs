...
async function findBundle() {
  ...
  const bundles = entries.filter(
    (entry) => entry.isDirectory() && /^notifyhandler-book012-admiralbet-[0-9a-f]{12}-win32-x64$/.test(entry.name),
  );
  if (bundles.length !== 1) {
...
export async function verifyPortableValidationBundle() {
  ...
  const [nodePin, automationPackage, manifest, runnerSource, launcher] = await Promise.all([
    ...
    readFile(join(bundle, "run-admiralbet-validation.cmd"), "utf8"),
  ]);
  ...
  if (
    ...
    manifest.bookmaker !== "admiralbet" ||
    manifest.approvedOrigin !== "https://www.admiralbet.it" ||
    manifest.startPath !== "/scommesse" ||
    ...
    throw new Error("Portable bundle manifest violates the reviewed DEVOPS-007 boundary.");
  }
  ...
  if (!runnerSource.includes('PORTABLE_BOOKMAKER = "admiralbet"')) {
    throw new Error("Portable runner is not locked to ADMIRALBET.");
  }
  if (!runnerSource.includes('PORTABLE_APPROVED_ORIGIN = "https://www.admiralbet.it"')) {
    throw new Error("Portable runner does not contain the exact approved ADMIRALBET origin.");
  }
  if (/https:\/\/www\.(?:sisal|bet365)\.it/i.test(runnerSource)) {
  ...
  if (/https?:\/\//i.test(launcher) || /\b(?:sisal|bet365)\b/i.test(launcher)) {
