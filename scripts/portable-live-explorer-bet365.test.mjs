...
import { windowsLauncherText } from "./build-bet365-live-validation-bundle.mjs";
import {
...
} from "./portable-live-explorer-bet365.mjs";

test("portable runner only exposes live BET365 mode and synthetic smoke", () => {
  ...
  for (const args of [
    ...
    assert.throws(() => parsePortableArguments(args), /locked to BET365/i);
  }
});
...
test("portable child environment locks bookmaker, browser path, and removes explorer overrides", () => {
  ...
  assert.equal(environment.NH_LIVE_EXPLORER_REQUIRE_RELAY, "1");
...
test("Windows launcher exposes no alternate bookmaker or origin selector", () => {
  ...
  assert.doesNotMatch(launcher, /\b(?:admiralbet|sisal)\b/i);
  assert.doesNotMatch(launcher, /NH_LIVE_EXPLORER_(?:BOOKMAKER|URL|MAX_ACTIONS|DELAY_MS)/i);
});

test("portable summary validator only accepts sanitized BET365 BOOK-012 summaries", () => {
  const base = {
...
test("portable embedded runtime pins match repository source-of-truth pins", async () => {
  ...
  assert.equal(PORTABLE_BOOKMAKER, "bet365");
  assert.equal(PORTABLE_APPROVED_ORIGIN, "https://www.bet365.it");
  assert.equal(PORTABLE_START_PATH, "/hub/it-it/football");
});
