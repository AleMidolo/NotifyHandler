...
const APPROVED_BOOKMAKER = "bet365";
const APPROVED_ORIGIN = "https://www.bet365.it";
const START_PATH = "/hub/it-it/football";
...
function operatorReadmeText(sourceCommit) {
  return [
    "NotifyHandler BOOK-012 BET365 diagnostic runner",
    "===================================================",
    "",
    ...
    "It is locked to the credential-free public origin https://www.bet365.it and reuses the reviewed BOOK-012 explorer unchanged.",
    "",
    `Source commit: ${sourceCommit}`,
    "",
    "Requirements:",
    "- Windows x64 workstation with a normal headed desktop session",
    "- outbound DNS/HTTPS access to www.bet365.it",
...
export async function buildPortableValidationBundle(options = {}) {
  ...
  const bundleName = `notifyhandler-book012-bet365-${shortSha}-win32-x64`;
  ...
  await cp(
    join(root, "scripts", "portable-live-explorer-bet365.mjs"),
  ...
  await writeFile(join(bundle, "run-bet365-validation.cmd"), windowsLauncherText(), "utf8");
