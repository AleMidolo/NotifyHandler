...
async function findBundle() {
  ...
  const bundles = entries.filter(
    (entry) => entry.isDirectory() && /^notifyhandler-book012-bet365-[0-9a-f]{12}-win32-x64$/.test(entry.name),
  );
  if (bundles.length !== 1) {
...
export async function verifyPortableValidationBundle() {
  ...
  const [nodePin, automationPackage, manifest, runnerSource, launcher] = await Promise.all([
    ...
    readFile(join(bundle, "run-bet365-validation.cmd"), "utf8"),
  ]);
  ...
  if (
    ...
    manifest.bookmaker !== "bet365" ||
    manifest.approvedOrigin !== "https://www.bet365.it" ||
    manifest.startPath !== "/hub/it-it/football" ||
    ...
    throw new Error("Portable bundle manifest violates the reviewed DEVOPS-010 boundary.");
  }
  ...
  if (!runnerSource.includes('PORTABLE_BOOKMAKER = "bet365"')) {
    throw new Error("Portable runner is not locked to BET365.");
  }
  if (!runnerSource.includes('PORTABLE_APPROVED_ORIGIN = "https://www.bet365.it"')) {
    throw new Error("Portable runner does not contain the exact approved BET365 origin.");
  }
  if (/https:\/\/www\.(?:admiralbet|sisal)\.it/i.test(runnerSource)) {
  ...
  if (/https?:\/\//i.test(launcher) || /\b(?:admiralbet|sisal)\b/i.test(launcher)) {
