...
import { windowsLauncherText } from "./build-live-validation-bundle.mjs";
import {
...
} from "./portable-live-explorer.mjs";

test("portable runner only exposes live ADMIRALBET mode and synthetic smoke", () => {
  ...
  for (const args of [
    ...
    assert.throws(() => parsePortableArguments(args), /locked to ADMIRALBET/i);
  }
});
...
test("Windows launcher exposes no alternate bookmaker or origin selector", () => {
  ...
  assert.doesNotMatch(launcher, /\b(?:sisal|bet365)\b/i);
  assert.doesNotMatch(launcher, /NH_LIVE_EXPLORER_(?:BOOKMAKER|URL|MAX_ACTIONS|DELAY_MS)/i);
});

test("portable summary validator only accepts sanitized ADMIRALBET BOOK-012 summaries", () => {
  const base = {
...
test("portable embedded runtime pins match repository source-of-truth pins", async () => {
  ...
  assert.equal(PORTABLE_BOOKMAKER, "admiralbet");
  assert.equal(PORTABLE_APPROVED_ORIGIN, "https://www.admiralbet.it");
  assert.equal(PORTABLE_START_PATH, "/scommesse");
});
